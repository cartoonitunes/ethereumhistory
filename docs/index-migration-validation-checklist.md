...
- [ ] `POST /api/cron/refresh-turso-totals` with the `CRON_SECRET` bearer token,
      then: (the full-index scan lives in this endpoint, not `refresh-stats` —
      it was split into its own cron slot before this branch landed)
