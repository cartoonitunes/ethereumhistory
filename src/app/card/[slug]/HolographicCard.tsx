...
export default function HolographicCard({
   ...
   * Share as one tweet.
   *
   * The link is the collection page, and it is doing the work: X unfurls it
   * into the large summary card that page already declares, so the image comes
   * along without anyone attaching anything. Web intents cannot attach media,
   * so an unfurl is the only way an image reaches the post automatically, and
   * the collection page is the better landing spot anyway. Someone who wants
   * the card PNG itself has Download PNG and Copy image beside this, which are
   * their own actions and not part of this flow.
   */
  ...
  // A preview has no collection page, so it points at itself. That page carries
  // its own unfurl image, so the post still arrives with a card.
  const tweetUrl = `https://x.com/intent/tweet?text=${encodeURIComponent(
    tweetText
  )}&url=${encodeURIComponent(collectionUrl ?? shareUrl)}`;
