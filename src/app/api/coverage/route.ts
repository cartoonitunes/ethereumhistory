      ...
      if (isDatabaseConfigured()) {
        ...
        const [neonByEra, neonByYear] = await Promise.all([
          db.execute(sql`
            ...
            FROM contracts
            WHERE short_description IS NOT NULL AND short_description != '' AND era_id IS NOT NULL
            GROUP BY era_id
          `),
          db.execute(sql`
            SELECT EXTRACT(YEAR FROM deployment_timestamp)::int as year, COUNT(*)::int as count
            FROM contracts
            WHERE short_description IS NOT NULL AND short_description != '' AND deployment_timestamp IS NOT NULL
            GROUP BY year
          `),
        ]);
